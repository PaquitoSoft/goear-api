...
exports.lookupTracks = function(tracks, onComplete) {
	...
	tracks.forEach(function(track) {
		requests.push(function(callback) {
			var searchUrl = substitute(serviceConfig.lookupBaseUrl, {
				trackId: track.id
			});
			// util.log("Let's look for extended info fo track: " + track.id);
			request(searchUrl, function(error, response, body) {
				if (error) {
					callback(error);
				} else if (response.statusCode !== 200) {
					...
					// util.log("Extended info received for track: " + track.id);
					...
					if (trackData.length > 0) {
						...
						callback(new Error("No extended info found for trackId: " + track.id));
					}
				}
			});
		});

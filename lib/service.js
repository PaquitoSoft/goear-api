	...
	request(searchUrl, function(error, response, body) {
		if (error) {
			...
			var $ = cheerio.load(body),
				result = {
					tracks: [],
					totalCount: parseInt($('#sounds_tab').text().split(' ')[0], 10)
				},
				searchResult,
				trackId;
			...
			if (result.totalCount > 0) {
				$('#search_results > li').each(function(index, item) {
					searchResult = $(this);
					trackId = searchResult.find('a').attr('href').match(/listen\/(.*)\//)[1];
					result.tracks.push({
						id: trackId,
						title: searchResult.find('.song').text().trim(),
						artist: searchResult.find('.group').text().trim(),
